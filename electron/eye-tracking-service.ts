...
export interface EyeTrackingConfig {
  ...
  sensitivity: number // multiplier for movement
...
export class EyeTrackingService {
   ...
   */
  updateConfig(config: Partial<EyeTrackingConfig>) {
   ...
   */
  async moveCursorFromGaze(gazeX: number, gazeY: number) {
    ...
    try {
      ...
      if (this.lastPosition) {
        const movement = Math.sqrt(
          Math.pow(targetX - this.lastPosition.x, 2) + 
  ...
  private async moveCursorNative(x: number, y: number): Promise<void> {
    if (isMacOS) {
...
// Singleton instance
let eyeTrackingService: EyeTrackingService | null = null

export function getEyeTrackingService(): EyeTrackingService {
